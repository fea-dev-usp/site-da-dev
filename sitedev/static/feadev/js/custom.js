(function () {
	...
	var app = {
		...
		appendBtnTop: function () {
			$("body").append(
				'<div class="btn-top"><svg class="btn-icon-right" viewBox="0 0 13 9" width="13" height="9"><use xlink:href="assets/img/sprite.svg#arrow-right"></use></svg></div>',
			);
		},
		...
		scrollTo: function () {
			$("html, body").animate(
				{ scrollTop: $($(this).attr("data-scroll-to")).position().top },
				1000,
			);
		},
			...
			handler: function () {
				...
				if (scrollTop >= offsetTop) {
					if (!app.headerFixed.IS_FIXED) {
						...
						header.after(
							'<div class="header-fixed-static" style="height:' +
								height +
								'px"></div>',
						);
					}
			...
			spincrement: function () {
				...
				if (counters.length) {
					jQuery.each(counters, function () {
						...
						if (
							$(window).scrollTop() >
								_this.offset().top - $(window).height() * 0.85 &&
